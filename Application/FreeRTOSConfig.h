...
/*includes and External variables used in trace hook macros */
#include"std_types.h"
#include"Timer0.h"
#include"Application.h"

#if(RUN_TIME_MEASURMENT_TASK_ENABLE == APPLICATION_TASK_ON)
extern uint32 TaskInTimeStamp[];
extern uint32 TaskOutTimeStamp[];
extern uint32 TaskExcutionTicksNum[];
extern uint32 TotalTasksTicks;
/* Trace hook macros when task is entered */
#define traceTASK_SWITCHED_IN()                                       \
do                                                                    \
{                                                                     \
 if(((uint32)(pxCurrentTCB->pxTaskTag)) != 0)                         \
 {                                                                    \
TaskInTimeStamp[(uint32)(pxCurrentTCB->pxTaskTag)] = GetTimer0Ticks();\
 }                                                                    \
}while(0);
/* Trace hook macros when task is exit */
#define traceTASK_SWITCHED_OUT()                                                                                                \
do                                                                                                                              \
{                                                                                                                               \
 if(((uint32)(pxCurrentTCB->pxTaskTag)) != 0)                                                                                   \
{                                                                                                                               \
TaskExcutionTicksNum[(uint32)(pxCurrentTCB->pxTaskTag)] = GetTimer0Ticks() - TaskInTimeStamp[(uint32)(pxCurrentTCB->pxTaskTag)];\
TotalTasksTicks += TaskExcutionTicksNum[(uint32)(pxCurrentTCB->pxTaskTag)];                                                     \
}                                                                                                                               \
}while(0);
#endif
